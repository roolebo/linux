...
static ssize_t
target_wwn_ieee_company_id_show(struct config_item *item,
				char *page)
{
	return snprintf(page, PAGE_SIZE, "%#08x\n",
			to_t10_wwn(item)->ieee_company_id);
}

static ssize_t
target_wwn_ieee_company_id_store(struct config_item *item,
				 const char *page, size_t count)
{
	struct t10_wwn *t10_wwn = to_t10_wwn(item);
	struct se_device *dev = t10_wwn->t10_dev;
...
CONFIGFS_ATTR(target_wwn_, ieee_company_id);
...
static struct configfs_attribute *target_core_dev_wwn_attrs[] = {
	...
	&target_wwn_attr_ieee_company_id,
